...
export default function Valuation() {
  ...
  // Mark dirty on any cap table change
  const setShareholdersWithDirty = (updater: Shareholder[] | ((prev: Shareholder[]) => Shareholder[])) => {
    setShareholders(updater);
    setDirty(true);
  };
  const setTotalSharesWithDirty = (val: number) => {
    setTotalSharesPool(val);
    setDirty(true);
  };
  ...
  const addShareholder = () => {
    if (atLimit) return;
    setShareholdersWithDirty(prev => [...prev, { id: `sh-${Date.now()}`, name: '', type: 'Investor', ownershipPct: 0, entryDate: '' }]);
  };
  const removeShareholder = (id: string) => setShareholdersWithDirty(prev => prev.filter(s => s.id !== id));
  const updateShareholder = (id: string, field: keyof Shareholder, value: string | number) => {
    setShareholdersWithDirty(prev => prev.map(s => s.id === id ? { ...s, [field]: value } : s));
  };
  ...
  const ebitdaValuations = useMemo(() => activeYears.map(y => {
    const ebitda = resolveAnnualMetric('EBITDA', y, projections.ebitda[y]);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl md:text-2xl font-bold text-primary">Valuation & Cap Table</h2>
                ...
                <tbody>
                  {ebitdaValuations.map(v => (
                    <tr key={v.year} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                      ...
                      <td className="p-2 text-right tabular-nums text-foreground">{formatCurrency(v.ebitda * 1000)}</td>
                      <td className="p-2 text-right tabular-nums font-medium text-primary">{formatCurrency(v.valuation * 1000)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {arrValuations.map(v => (
                    <tr key={v.year} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                      <td className="p-2 text-foreground">{v.year}</td>
                      <td className="p-2 text-right tabular-nums text-foreground">{formatCurrency(v.mrr * 1000)}</td>
                      <td className="p-2 text-right tabular-nums text-foreground">{formatCurrency(v.arr * 1000)}</td>
                      <td className="p-2 text-right tabular-nums font-medium text-primary">{formatCurrency(v.valuation * 1000)}</td>
                    </tr>
                  ))}
                </tbody>
