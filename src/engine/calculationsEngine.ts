...
import { Year, YEARS, Assumptions, DEFAULT_ASSUMPTIONS, Scenario, TicketKey, BUTaxConfig, SubProductTaxConfig, ALL_SUBPRODUCT_KEYS, getSubProductTaxRate, CosConfig, DEFAULT_COS_CONFIG, MRR_KEYS, computeMixPresumido, getEffectivePresumido } from '@/lib/financialData';
...
function getTicketForMonth(product: TicketKey, month: number, year: number, assumptions: Assumptions): number {
  // Check monthly overrides first
  const monthlyArr = assumptions.monthlyTickets?.[product]?.[year as Year];
  if (monthlyArr && monthlyArr[month] !== undefined && monthlyArr[month] !== null) {
    return monthlyArr[month];
  }
  // Fallback to flat ticket value
  return assumptions.tickets[product];
}

// ─── REVENUE ───

function calcMonthlyRevenue(month: number, year: number, assumptions: Assumptions, scenarioMult: number) {
  const caasAssessoria = getMonthlyClientCount('caas', 'assessoria', month, year, assumptions) * getTicketForMonth('caasAssessoria', month, year, assumptions);
  const caasEnterprise = getMonthlyClientCount('caas', 'enterprise', month, year, assumptions) * getTicketForMonth('caasEnterprise', month, year, assumptions);
  const caasCorporate  = getMonthlyClientCount('caas', 'corporate', month, year, assumptions) * getTicketForMonth('caasCorporate', month, year, assumptions);
  const caasSetup      = getMonthlyClientCount('caas', 'setup', month, year, assumptions) * getTicketForMonth('caasSetup', month, year, assumptions);
...
function calcMonthlyHeadcount(
  ...
  if (year === 2025) {
    if (month === 0) basePay += 3067 + 7500; // Temp employees Jan only
  }
  let salaries = basePay / 1000 * yearMult;

  // Item 7: Squad config is SUBSTITUTIVE — when active, replaces ratio-based hiring
  const sq = assumptions?.squadConfig;
  let totalHC: number;
  ...
  if (sq) {
    ...
    const baseHC = namedEmployees2025.filter(e => !('endMonth' in e)).length;
    totalHC = baseHC + additionalCFOs + additionalFPA + additionalCSM + additionalPF +
