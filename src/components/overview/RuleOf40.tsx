...
// ---------------------------------------------------------------------------
// Period-aware helpers (shared logic with Overview)
// ---------------------------------------------------------------------------
...
export default function RuleOf40() {
  const { projections, filteredYears, focalYear } = useFinancialModel();
  ...
  const chartData = activeYears.map((y, i) => {
    ...
    const prev = prevYear != null ? getKpiValue('RECEITA BRUTA', prevYear as Year, projections.grossRevenue[prevYear as Year]) : 0;
    ...
    const profitMargin = yr_netRevenue > 0 ? (yr_ebitda / yr_netRevenue) * 100 : 0;
  ...
  const currentData = chartData.find(d => d.year === focalYear.toString()) ?? chartData[chartData.length - 1];
      ...
      <div className="kpi-card relative">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="top" className="max-w-[240px] text-xs">
                Rule of 40 = Revenue Growth % + EBITDA Margin %. A SaaS metric used by investors to evaluate the balance between growth and profitability.
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        <div className="mt-2 space-y-0.5 text-[11px]">
          ...
          <div className="flex items-center gap-1.5 text-muted-foreground">
            ...
            <span>EBITDA Margin: <span className="font-semibold text-foreground">{profitMargin.toFixed(1)}%</span></span>
          </div>
        </div>
      </div>
